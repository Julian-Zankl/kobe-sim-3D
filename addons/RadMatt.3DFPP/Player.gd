...
# INTERACTIONS
	...
	if $Yaw/Camera3D/InteractionRay.is_colliding():
		var x = $Yaw/Camera3D/InteractionRay.get_collider()
...
# VECTOR 3 for where the player is currently looking
	...
	var dir = (get_node("Yaw/Camera3D/look_at").get_global_transform().origin - get_node("Yaw/Camera3D").get_global_transform().origin).normalized()
	look_vector = dir
...
func _process_movements(delta):
	...
	hvel = hvel.lerp(target, accel * move_speed * delta)
	velocity.x = hvel.x
	velocity.z = hvel.z
...
func _unhandled_input(event):
	if event is InputEventMouseMotion:
		...
		$Yaw.rotation = Vector3(0, deg_to_rad(yaw), 0)
		$Yaw/Camera3D.rotation = Vector3(deg_to_rad(pitch), 0, 0)
